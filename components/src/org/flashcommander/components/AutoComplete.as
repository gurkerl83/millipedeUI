...
//		public var inputTxt:TextInput;
		...
		public var contextButton:Button;
...
//				inputTxt.text = _text;
//			}
			if (instance==contextButton){
				contextButton.addEventListener(FocusEvent.FOCUS_OUT, _focusOutHandler)
				contextButton.addEventListener(FocusEvent.FOCUS_IN, _focusInHandler)
				contextButton.addEventListener("keyDown", onKeyDown);
				contextButton.addEventListener(FlexEvent.ENTER, enter);
			}
			if (instance==list){
...
//			if (!this.focusManager || this.focusManager.getFocus()!=inputTxt) return;
			if (!this.focusManager || this.focusManager.getFocus()!=contextButton) return;
...
//				popUp.width = contextButton.width;
				popUp.width = 100;
//			}
		}
...
//				if (inputTxt.hitTestPoint(e.stageX, e.stageY)) return;
				if (contextButton.hitTestPoint(e.stageX, e.stageY)) return;
			}
			
			close(event as FocusEvent);
		}
		
		private function close(event:FocusEvent):void{
			popUp.displayPopUp = false;
		}
...
//			if (inputTxt) inputTxt.enabled = value;
			if (contextButton) contextButton.enabled = value;
		}
