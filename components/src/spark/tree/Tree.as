...
public class Tree extends List
{
	...
	override public function set dataProvider(value:IList):void
	{
		if (value)
		{
			_dataProvider = new TreeDataProvider(value);
			_dataProvider.dataDescriptor = dataDescriptor;
		}
		else
		{
			value = null;
	...
	public function expandItem(item:Object, open:Boolean = true):void
	{
		if (dataDescriptor.hasChildren(item))
		{
			...
			if (open)
				_dataProvider.openBranch(children, item);
			else
				_dataProvider.closeBranch(children, item);
		}
	}
